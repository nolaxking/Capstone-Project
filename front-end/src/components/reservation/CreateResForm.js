...
import { createRes, updateRes, readRes } from "../../utils/api";
...
export default function CreateResForm({ type }) {
  ...
  useEffect(() => {
    if (type === "Edit") {
      const loadForm = async () => {
        const newRes = await readRes(reservation_id);
        setFormData({
          ...newRes,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (type === "Edit") {
        await updateRes(reservation_id, { data: formData });
      } else {
        await createRes({ data: formData });
      }
